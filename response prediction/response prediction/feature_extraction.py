...
def collect_userInfo():	
	...
	conn=MySQLdb.connect(host='localhost',user='root',passwd='root',db='facebook',port=3306)
	cur=conn.cursor()
	cur.execute('select id, followers_count, friends_count, listed_count, favourites_count, statuses_count from facebook_user order by id')
	count=0
...
def collect_tweets():
	...
	# check tweets
	cur.execute('select user_id, id, hour(created_at) as tweet_hour from facebook_tweets order by user_id')
	count=0
	while True:
		row = cur.fetchone()
		if row == None:
