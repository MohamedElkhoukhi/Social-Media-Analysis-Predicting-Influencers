...
## You can learn more about OAuth here: https://dev.facebook.com/docs/auth/oauth
## DanielMa
...
def main( mode = 1 ):
	user_sucess = open('./log/posts-success-1.txt', 'r')
	user_ok = []
	...
	success_recorder = open('./log/posts-success-1.txt', 'a')
	for user_id in user_ids:
		kargs = {}
		print 'collect user:%s'%user_id
		output = open('./newyork-posts/' + user_id, 'w')
		try: 
			post_count = 0
			# since_id post_id after 7:54 PM - 24 Dec 2013
			for status in Cursor(api.user_timeline, None, user_id=user_id, count=200, include_rts='true', since_id=415450313802674176).items(3200):
				...
				if post_count%200 == 0:
					print '%s posts have been collected...'%post_count
			...
			continue		
		print 'done!user_id:%s,post_count:%s'%(user_id, post_count)
...
def process_status(status):
	...
	if 'reposted_status' in result:
		result['reposted_status'] = process_status(result['reposted_status'])
	if 'place' in result:
		del result['place']
	return result
