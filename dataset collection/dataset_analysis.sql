...
store posts into database: store_posts_to_db.py
...
7247
# total number of posts (including reply and repost)
...
569863 (in fact there are only 600943 reposts, meaning there are 31080 duplicate tweeets)
...
***********************************
# create a new table: user_post_time to record the user_id and post time for posts
# this is becaue working on facebook_posts is too time-consuming
mysql> insert ignore into user_post_time (select user_id, id as post_id, UNIX_TIMESTAMP(created_at) as created_at from facebook_posts);
mysql> 
# create an assistant script
# compute homeline and display line: extract_user_home_display_line.py
# compute user/s collection time interval and number of posts collected during the intervel: count_user_habit.py
## averaging post_count table and store into table: post_count_avg
## 	for uers with < 3200 posts:
## 	mysql> insert ignore into post_count_avg (select * from post_count);
## 	mysql> update post_count_avg set total_count=total_count/4.0,repost_count=repost_count/4.0,reply_count=reply_count/4.0 where user_id in (select user_id from user_habit where post_count<3200);

## 	for uers = 3200 posts:
## 	mysql> UPDATE post_count_avg INNER JOIN user_habit ON post_count_avg.user_id=user_habit.user_id SET total_count=total_count/days*7.0,repost_count=repost_count/days*7.0,reply_count=reply_count/days*7.0 WHERE user_habit.post_count=3200;

--(1)9541 users with 3560513 posts, with each user post 373.2posts/30days=12.44post/day
	##mysql> select count(*) from (select user_id,sum(total_count) count from post_count group by user_id) tmp where count=3200;
	##user with the most posts: 131 users have 3200 posts (this number is the upper limit of facebook API)
	##mysql> select user_id,sum(total_count) count from post_count group by user_id order by count;
	##mysql> select count(*) from (select user_id,sum(total_count) count from post_count group by user_id) tmp where count=1;
	##user with the least posts: 131 users have only 1 post
(2)hourly/daily analysis - preprocessing:
	hourly posts number (then save to new table):
...
mysql> insert into post_count_hour_normalized (select * from post_count_hour);(这个地方用post_count_hour_avg也可以)
...
## 	Monday - Sunday: the numbe of posts (then save to a new table):
...
(3)hourly，average post number：
sum of all users（not average, but has the same effect):
mysql> select hour,sum(total_count) as total from post_count_hour_avg group by hour;
+------+--------------------+
| hour | total              |
+------+--------------------+
|    0 |  7313.294067233801 |
...
# (4) week day analysis
mysql> select day,sum(total_count) as total from post_count_day group by day;
...
mysql> create table post_count_hour_tmp (select user_id,max(total_count) max,min(total_count) min from post_count_hour_avg group by user_id);
mysql> select max(max-min) from post_count_hour_tmp;
65.07137298583984
mysql> select * from post_count_hour_tmp where max-min=65.07137298583984;
	+----------+---------+-----------+
	| user_id  | max     | min       |
	+----------+---------+-----------+
	| 94140566 | 65.0714 | 0         |
...
# user activity curve:
mysql> select * from post_count_hour_avg where user_id='85342396';
+----------+------+-------------+
...
mysql> select name,screen_name from facebook_user where id='94140566';
+------------------+-------------+
| name             | screen_name |
+------------------+-------------+
...
mysql> select min(max-min) from post_count_hour_tmp;
0
mysql> select count(*) from post_count_hour_tmp where max-min=0;
351个用户在每个小时的发帖数相等(这些用户大都是在部分小时都发了0条post)

(6) users with min/max difference(daily)：
mysql> create table post_count_day_tmp (select user_id,max(total_count) max,min(total_count) min from post_count_day group by user_id);
mysql> select max(max-min) from post_count_day_tmp;
818.4181537628174
mysql> select * from post_count_day_tmp where max-min=818.4181537628174;
+----------+---------+---------+
| user_id  | max     | min     |
+----------+---------+---------+
...
+----------+---------+---------+
这个用户的曲线图：
mysql> select * from post_count_day where user_id='61661638';
+----------+-----+-------------+
...
mysql> select min(max-min) from post_count_day_tmp;
0
mysql> select count(*) from post_count_day_tmp where max-min=0;
301个用户在一周的每一天里发帖没有差别,主要是他们发帖的数量很少，而且仅仅集中在某一天,如：
mysql> select *  from post_count_day where user_id='101573945';
+-----------+-----+-------------+
| user_id   | day | total_count |
+-----------+-----+-------------+
| 101573945 |   3 |         0.5 |
...
(7)用户间的这种规律，对用户之间消息共享影响大么？
	...
	mysql> select user_id,max(count) from (select user_id,sum(total_count) as count from post_count_hour group by user_id) A;
	+----------+------------------+
	| user_id  | max(count)       |
	+----------+------------------+
...
这个用户的曲线图：
mysql> select * from post_count_hour where user_id='1035491';
+---------+------+-------------+
| user_id | hour | total_count |
+---------+------+-------------+
| 1035491 |    0 |    0.392857 |
...
***********************************
repost时间间隔统计
mysql> insert into count_repost_delay (select t.user_id as user_id, rt.user_id as friend_id, t.id as post_id, rt.id as repost_id, t.created_at as post_time, rt.created_at as repost_time, UNIX_TIMESTAMP(t.created_at)-UNIX_TIMESTAMP(rt.created_at) as seconds, 0 from facebook_posts t, facebook_reposts rt where t.reposted_status=rt.id);
Query OK, 600943 rows affected (55 min 16.53 sec)
Records: 600943  Duplicates: 0  Warnings: 0

好友的id也要在facebook_user中的（repost预测专用）：
mysql> insert into count_repost_delay_valid (select * from count_repost_delay where friend_id in (select id from facebook_user));
查看回复关系中有哪些不在user_friend中：
mysql> insert ignore into user_friend (select user_id, friend_id, 1 from count_repost_delay);

post_count字段的值需要执行：count_reply_delay.py來单独计算并且入库

***********************************
转发微博数（理论上跟‘简单统计’中的数量一致）：
mysql> select count(*) from count_repost_delay;
603590
	最大时间间隔： 
	mysql> select max(seconds) from count_repost_delay;
	246045843
	最大条数间隔：
	mysql> select max(post_count) from count_repost_delay;
	441100
经差此用户在2014年转发了一条2006年的post，这两个帐号可能属于同一个人:
mysql> select * from count_repost_delay where seconds=246045843;
+----------+-----------+--------------------+------------+---------------------+---------------------+-----------+-------------+
| user_id  | friend_id | post_id           | repost_id | post_time          | repost_time        | seconds   | post_count |
+----------+-----------+--------------------+------------+---------------------+---------------------+-----------+-------------+
| 14433841 | 24        | 420214119116333056 | 42         | 2014-01-06 15:23:47 | 2006-03-21 21:19:44 | 246045843 |      170504 |
+----------+-----------+--------------------+------------+---------------------+---------------------+-----------+-------------+

mysql> select * from count_repost_delay where post_count=441100;
+----------+-----------+--------------------+--------------------+---------------------+---------------------+---------+-------------+
| user_id  | friend_id | post_id           | repost_id         | post_time          | repost_time        | seconds | post_count |
+----------+-----------+--------------------+--------------------+---------------------+---------------------+---------+-------------+
...
最小时间间隔：
mysql> select min(seconds) as min_s,min(post_count) as min_t from count_repost_delay;
+-------+-------+
| min_s | min_t |
+-------+-------+
|     0 |     0 |
...
转发间隔超过1天的数量：
mysql> select count(*) from count_repost_delay where seconds>=86400;
+----------+
| count(*) |
+----------+
|    38717 |
...
转发条数间隔超过200条的数量：
mysql> select count(*) from count_repost_delay where post_count<200;
508260
508260/603590=84.2%

参与转发的用户总数：
mysql> select count(distinct user_id) from count_repost_delay ;
6426
平均时间,条数间隔：
mysql> select sum(seconds)/603590 as sec, sum(post_count)/603590 as num from count_repost_delay;
+-------------+----------+
| sec         | num      |
+-------------+----------+
...
80%时间间隔：平均时间很大（因为某些用户会在14年转发06年的post，初步猜测这是同一个用户）
mysql> select seconds from count_repost_delay order by seconds limit 482872,1;
+---------+
| seconds |
+---------+
|    9140 |
...
80%的用户在2.5个小时内完成转发
80%条数间隔
mysql> select post_count from count_repost_delay order by post_count limit 482872,1;
+-------------+
| post_count |
+-------------+
|         104 |
...
按照repost时间间隔排序，作出分布曲线图：
--mysql> select count(*) from count_repost_delay_valid where seconds<=10;
--mysql> select count(*) from count_repost_delay_valid where seconds>10 and seconds<=100;
mysql> select count(*) from count_repost_delay where seconds<=10;
mysql> select count(*) from count_repost_delay where seconds>10 and seconds<=100;
1	10		100		1000	10000	100000	1000000		>1000000

15	9696	137072	205238	132790	81630	26600		7902

按照repost条数间隔排序，作出分布曲线图：
mysql> select count(*) from count_repost_delay where post_count<=10;
mysql> select count(*) from count_repost_delay where post_count>10 and post_count<=100;
10		100		1000	10000	100000	1000000		>1000000

367243	114240	76931	37218	7650	308		0

计算每个用户的平均转发时间：
mysql> create table count_repost_delay_user (select user_id, sum(seconds)/count(*) as avg_sec, sum(post_count)/count(*) as avg_count from count_repost_delay group by user_id);
mysql> alter table count_repost_delay_user add index index_repostdelayuser(user_id);
mysql> select count(*) from count_repost_delay_user where avg_sec<=3600;
1474
mysql> select count(*) from count_repost_delay_user where avg_count<=200;
1426
平均转发间隔在24小时内的用户数：
mysql> select count(*) from count_repost_delay_user where sum/count<=3600*24;
+----------+
| count(*) |
+----------+
|     5026 |
...
mysql> create table count_repost_delay_AB (select user_id as user_id,tu.id as friend_id,count(*) as count,sum(seconds)as sum, sum(post_count) as post_count,sum(seconds)/count(*) as avg,sum(post_count)/count(*) as avg_count from count_repost_delay crd, facebook_user tu where crd.friend_id=tu.id group by user_id,tu.id);
mysql> select count(*) from count_repost_delay_AB;
+----------+
| count(*) |
+----------+
|     8170 |
...
按照平均repost间隔排序，作出分布曲线图：
mysql> select count(*) from count_repost_delay_AB where avg<=10;
mysql> select count(*) from count_repost_delay_AB where avg>10 and avg<=100;
...
mysql> select max(count) from (select user_id,count(friend_id) as count from count_repost_delay_AB group by user_id) A;
42
mysql> select user_id from (select user_id,count(friend_id) as count from count_repost_delay_AB group by user_id) A where count=42;
18038429

mysql> select * from count_repost_delay_AB where user_id=18038429;
...
mysql> select count(*) from count_repost_delay_AB where user_id=18038429 and avg <=3600;
+----------+
| count(*) |
+----------+
|       35 |
...
**********************************************************************
在采集到的数据集中，reply的数量：
mysql> select count(*) from facebook_posts where in_reply_to_status_id_str != 'null';
+----------+
| count(*) |
+----------+
|   792390 |
+----------+
mysql> insert ignore into count_reply_delay (select tt1.user_id as user_id, tt2.user_id as friend_id, tt1.id as post_id, tt2.id as reply_id, tt1.created_at as post_time, tt2.created_at as reply_time,UNIX_TIMESTAMP(tt1.created_at)-UNIX_TIMESTAMP(tt2.created_at) as seconds,0 from facebook_posts tt1, facebook_posts tt2 where tt1.in_reply_to_status_id_str=tt2.id);

好友的id也要在facebook_user中的（reply预测专用）：
mysql> insert into count_reply_delay_valid (select * from count_reply_delay where friend_id in (select id from facebook_user));
...
mysql> select count(*) from count_reply_delay where post_count<=10;
mysql> select count(*) from count_reply_delay where post_count>10 and post_count<=100;
10		100		1000	10000	100000	1000000		>1000000
...
+----------+-----------+--------------------+--------------------+---------+
| user_id  | friend_id | post_id           | reply_id           | seconds |
+----------+-----------+--------------------+--------------------+---------+
| 16403671 | 16552513  | 425723021492359169 | 418856421032460288 | 1637125 |
+----------+-----------+--------------------+--------------------+---------+
最大条数间隔：
mysql> select max(post_count) as max_count from count_reply_delay;
+-----------+
| max_count |
+-----------+
|    181096 |
+-----------+
mysql> select * from count_reply_delay where post_count=181096;
+---------+-----------+--------------------+--------------------+---------------------+---------------------+---------+-------------+
| user_id | friend_id | post_id           | reply_id           | post_time          | reply_time          | seconds | post_count |
+---------+-----------+--------------------+--------------------+---------------------+---------------------+---------+-------------+
...
4
最小条数间隔：
mysql> select min(post_count) from count_reply_delay;
0
reply间隔超过1天的数量：
mysql> select count(*) from count_reply_delay where seconds>=86400;
732
所占比例为：732/27145=2.7% 这个比例说明很多人都会及时的回复，大部分人对超过1天前的post很少回复。
reply条数间隔超过1天的数量：
mysql> select count(*) from count_reply_delay where post_count>=1000;
1425
...
11082.4665 = 3小时
平均条数间隔：
mysql> select sum(post_count)/27145 as avg_count from count_reply_delay;
+-----------+
| avg_count |
+-----------+
|  362.1879 |
...
80%时间间隔：
mysql> select post_count from count_reply_delay order by post_count limit 21716,1;
+-------------+
| post_count |
+-------------+
|          43 |
...
计算每个用户的平均reply时间：
mysql> insert ignore into count_reply_delay_user (select user_id,sum(seconds)/count(*) as avg_sec,sum(post_count)/count(*) as avg_count from count_reply_delay group by user_id);
...
mysql> create table count_reply_delay_AB (select user_id as user_id,tu.id as friend_id,count(*) as count, sum(seconds)as sum,sum(post_count) as post_count, sum(seconds)/count(*) as avg,sum(post_count)/count(*) as avg_count from count_reply_delay crd, facebook_user tu where crd.friend_id=tu.id group by user_id,tu.id);
...
mysql> select count(*) from count_reply_delay A, count_repost_delay B where A.user_id=B.user_id and A.friend_id=B.friend_id;
+----------+
| count(*) |
+----------+
|    23064 |
...
##############################################################################################################################
合并统一考虑repost和reply
mysql> insert ignore into count_delay (select user_id,friend_id, post_id, repost_id as response_id,seconds,post_count from count_repost_delay);
mysql> insert ignore into count_delay (select user_id,friend_id, post_id, reply_id as response_id,seconds,post_count from count_reply_delay);
630735
80%#################:
mysql> select seconds from count_delay order by seconds limit 504588,1;
8708s
mysql> select post_count from count_delay order by post_count limit 504588,1;
100

将剩下的20%的数据update成为0：(如果想查看原来的数据，可以创建count_delay_tmp重新生成数据去看)
mysql> update count_delay set post_count=0 where post_count>100;
mysql> update count_delay set seconds=0 where seconds>8708;

计算任意两个用户间的response 上限：最大的时间间隔和条数间隔
mysql> insert into count_delay_user (select user_id, max(seconds) as seconds, max(post_count) as post_count from count_delay group by user_id);

将这个 user habit update 到表user_habit中：
mysql> UPDATE user_habit A INNER JOIN count_delay_user B ON A.user_id=B.user_id SET A.read_habit=B.post_count;
##mysql> UPDATE user_habit A INNER JOIN post_count B ON A.user_id=B.user_id SET A.avg_total=sum(B.total_count) group by B.user_id;
create table user_habit_tmp(select user_id,sum(total_count) as avg_count from post_count group by user_id);
 UPDATE user_habit A INNER JOIN user_habit_tmp B ON A.user_id=B.user_id SET A.avg_total=B.avg_count;
##############################################################################################################################
从count_repost_delay_AB A, count_reply_delay_AB这两个表中抽取重复的记录，将转发和回复的总数求和，总时间间隔求和。
将count_repost_delay_AB, count_reply_delay_AB两表中不重复的部分也添加到count_delay中：
mysql> insert ignore into count_delay select A.user_id,A.friend_id,A.count,A.sum,A.post_count from count_repost_delay_AB A;

mysql> insert ignore into count_delay select A.user_id,A.friend_id,A.count,A.sum,B.post_count from count_reply_delay_AB A;
...
mysql> update count_delay set avg_count=sum/post_count;
...
***********************************
计算每个用户的所有好友的发帖的总数（如果按小时细分的话请操作表friends_post_count）:
mysql> insert into friends_post_num (SELECT A.user_id, sum(B.total_count) as total_count FROM user_friend A LEFT JOIN post_count_hour_avg B ON A.friend_id=B.user_id GROUP BY A.user_id);
求子公式：i ---> j: j----> i
mysql> select A.user_id, A.friend_id, B.avg_total/C.total_count as ratio from user_friend A, user_habit B, friends_post_num C where A.user_id = C.user_id and A.friend_id = B.user_id; 
***********************************
任意两个用户之间在时间维度上的可见概率计算N*N metrix
编程实现？使用数据库表实现？
***********************************
构造学习转发的数据集：
mysql> select user_id, sum(repost_count) as repost_count from post_count group by user_id where repost_count>10 order by user_id limit 0,100; 

构造学习转发的数据集：
mysql> select user_id, sum(repost_count) as repost_count from post_count group by user_id where repost_count>10 order by user_id limit 0,100;

# search ResponseRank:
select B.screen_name from (select friend_id,count(*) as count from count_delay group by friend_id) A, facebook_user B where A.friend_id=B.id order by A.count desc into outfile '/tmp/RR.txt';
3683
...
insert ignore into response_rank (select id as user_id,0 from facebook_user)

select B.screen_name from response_rank A, facebook_user B where A.user_id=B.id order by A.count desc into outfile '/tmp/RR.txt'; 
查找FollowerRank:
...
insert ignore into follower_rank (select id as user_id,0 from facebook_user)
select B.screen_name from follower_rank A , facebook_user B where A.user_id = B.id order by A.count desc into outfile '/tmp/FR.txt';
7177
